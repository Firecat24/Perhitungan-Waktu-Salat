...
def perhitungan_equation(Tahun, Bulan, B, Dzuhur):
    ...
    L0 = L00 - L02
    L0R = math.radians(L0)
    EoT = (-(1789 + 237 * T) * math.sin(L0R) - (7146 - 62 * T) * math.cos(L0R) + (9934 - 14 * T) * math.sin(2 * L0R) - (
                29 + 5 * T) * math.cos(2 * L0R) + (74 + 10 * T) * math.sin(3 * L0R) + (320 - 4 * T) * math.cos(
        3 * L0R) - 212 * math.sin(4 * L0R)) / 1000
    Jam = 0
    ...
    C = (1.914602 - (0.004817 * T) - 0.000014 * (T ** 2)) * math.sin(Mrad) + (0.019993 - (0.000101 * T)) * math.sin(
    ...
    delta_obliquity1 = ((9.20 / 3600) * math.cos(omegarad)) + (0.57 / 3600) * math.cos(2 * Lrad) + (
                0.10 / 3600) * math.cos(2 * L1rad) - (0.09 / 3600) * math.cos(2 * omegarad)
    epsilon0 = (23 * 3600) + (26 * 60) + 21.448 - 4680.93 * U - 1.55 * (U ** 2) + 1999.25 * (U ** 3) - 51.38 * (
                U ** 4) - 249.67 * (U ** 5) - 39.05 * (U ** 6) + 7.12 * (U ** 7) + 27.87 * (U ** 8) + 5.79 * (
                           U ** 9) + 2.45 * (U ** 10)
...
def perhitungan_degree(rlintang, rdeklinasi, rtinggi):
    t = math.degrees(math.acos(
        -math.tan(rlintang) * math.tan(rdeklinasi) + 1 / math.cos(rlintang) * 1 / math.cos(rdeklinasi) * math.sin(
            rtinggi)))
    return t
